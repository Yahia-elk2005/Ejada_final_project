  ...
  return (
    <Card className="border-0 h-100 bg-transparent rounded-0 d-flex flex-column">
      <Link to={`/product/${productId}`} className="text-decoration-none">
        <div className="position-relative overflow-hidden mb-3" style={{ height: '360px' }}>
          <Card.Img 
            variant="top" 
      ...
      <Card.Body className="p-0 d-flex flex-column flex-grow-1">
        ...
        <div className="mb-3">
          {product?.originalPrice ? (
            ...
            <span className="text-secondary small fw-bold">${product?.price || '300.000'}</span>
          )}
        </div>
        ...
        <div className="mt-auto">
          <Link
            to={``}
            className="btn btn-teal rounded-0 text-white text-decoration-none d-inline-block"
            style={{
              width: 'fit-content',
              fontSize: '0.75rem',
              letterSpacing: '1px',
